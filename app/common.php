 ...
 * @return array|mixed
 */
function get_menu_byid($id=0,$self=false,$open=0,$field='id')
{
    if(empty($open)){
        $where['menu_open']=0;
    }elseif($open==1){
...
function menu_text()
{
	...
	if(empty($menu_text)){
		$menu_text=Db::name('menu')->where('menu_type <> 4')-> order('listorder') -> select();
...
/**
 * 返回选定父级ID的菜单数组
 * @param string $tag  查询标签，以字符串方式传入,例："cid:1,2;field:title,content;limit:3,8;order:listorder desc;"
 *  pid:父级ID
 ...
 *  order:排序方式，如：hits desc<br>
 *  where:查询条件，字符串形式，和sql语句一样
 * @return array|mixed
 */
function get_menu_album($tag)
{
    $tag=param2array($tag);
    $field = !empty($tag['field']) ? $tag['field'] : '*';
    $limit = !empty($tag['limit']) ? $tag['limit'] : '';
    $order = !empty($tag['order']) ? $tag['order'] : 'listorder';
    $config=array();
    //根据参数生成查询条件
    if (!empty($tag['cid'])) {
        $where['id'] = array('in',$tag['cid']);
    }
    if (!empty($tag['pid'])) {
        $where['parentid'] = $tag['pid'];
    }else{
    	$where['parentid'] = '0';
    }
    $menu=Db::name("menu")->field($field)->where($where)->order($order)->limit($limit)->select();
    return $menu;
}

/**
 * 返回指定id的菜单-前端菜单
 * app\home\taglib\YunCMF.php
...
{
    $navs= Db::name("menu")->where(array('menu_open'=>1))->order(array("listorder" => "ASC"))->select();
    foreach ($navs as $key=>$nav){
		//处理菜单类型
		switch($nav['menu_type']){
			//文章栏目
			case '3':
			...
			//单页路由
			case '4':
				$nav['href']=url('home/Singlepage/index?id='.$nav['id']);
				break;
			default:
				$nav['href']=$nav['menu_address'];
		}
...
function get_menu_one($id)
{
    $rst=array();
    if($id){
        $rst=Db::name('menu')->where(array('menu_type'=>4,'id'=>$id))->find();
    }
    return $rst;
}
/**
...
function get_email_options()
{
    ...
    if(empty($email_options)){
        $option = Db::name("Options")->where("option_name='email_options'")->find();
        if($option){
...
function get_active_options()
{
    ...
    if(empty($active_options)){
        $option = Db::name("Options")->where("option_name='active_options'")->find();
        if($option){
...
{
    // 判断短信模板是否存在
    $send_scene = Db::name("sms_template")->where(array("scene_code"=>$scene))->count('tpl_id');
    if (!$send_scene) {
        return array("status" => -1, "msg" => "请先添加[$scene]短信模板");
    }
    // 判断短信接口是否开启
    $sms_sys=sys_config_get('think_sdk_sms');
    $smsEnable = $sms_sys['sms_open'];
    if (!$smsEnable) {
        return array("status" => 0, "msg" => "未开启短信接口");
    }
...
function sendSms($scene, $mobile, $params)
{
    // 获取短信接口参数
    $sms_sys=sys_config_get('think_sdk_sms');
    $socketid = $sms_sys['AccessKeyId'];
    $socketpass = $sms_sys['accessKeySecret'];
    ...
    $sms_socket="http://39.108.163.217/SendSms.aspx";
    $post_data="uid=".$socketid."&pass=".$socketpass."&sjh=".$mobile."&memo=".$smscontent."&ftype=2"."&sendtime=".$time;
    // 以curl方式推送短信
    $ch = curl_init();  
    curl_setopt($ch, CURLOPT_URL, $sms_socket);  
    curl_setopt($ch, CURLOPT_HEADER,0);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true) ; // 获取数据返回
    curl_setopt($ch, CURLOPT_BINARYTRANSFER, true) ; // 在启用 CURLOPT_RETURNTRANSFER 时候将获取数据返回
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
    $sms_status = curl_exec($ch);
    curl_close($ch); 
    if (!empty($sms_status)) {
        if ($sms_status < 0) {
            $log_id = Db::name("sms_log")->insertGetId(array('mobile' => $mobile, 'code' => $code, 'add_time' => time(), 'session_id' => $session_id, 'status' => 0, 'scene' => $scene, 'msg' => $smscontent, 'error_msg' => $sms_status));
            return array('status'=>-1,'msg'=>'发送失败'.$sms_status,'log_id'=>$log_id);
        }else{
            $log_id = Db::name("sms_log")->insertGetId(array('mobile' => $mobile, 'code' => $code, 'add_time' => time(), 'session_id' => $session_id, 'status' => 1, 'scene' => $scene, 'msg' => $smscontent, 'error_msg' => $sms_status));
            $status_arr = explode('|', $sms_status);
            return array('status'=>1,'msg'=>'发送成功'.$status_arr[0],'log_id'=>$log_id);
        }
    }
}
/*************************************字符处理类********************************************************/
...
function export2excel($table,$file='',$fields='',$field_titles='',$tag='')
{
    ...
    /*设置excel的属性*/
    $objPHPExcel->getProperties()->setCreator("翟翟")//创建人
    ->setLastModifiedBy("翟翟")//最后修改人
...
function checkVersion()
{
    if(extension_loaded('curl')){
        $url = 'http://www.yuninf.cn/index.php?m=home&c=upgrade&a=check';
        $params = array(
                'version' => config('yuncmf_version'),
...
function check_user_action($object="",$count_limit=1,$ip_limit=false,$expire=0)
{
    ...
    if($find_log){
        ...
        Db::name("action_log")->where($where)->update(array("count"=>array("exp",Db::raw('count+1')),"last_time"=>$time,"ip"=>$ip));
    }else{
        Db::name("action_log")->insert(array("uid"=>$userid,"action"=>$action,"object"=>$object,"count"=>array("exp",Db::raw('count+1')),"last_time"=>$time,"ip"=>$ip));
    }
    return true;
}
/**
...
function get_imgurl($url,$cat=0)
{
    if(stripos($url,'http')!==false){
       ...
       //头像
        ...
        return __ROOT__.'/public/images/'.$imgurl;
    }else{
        //本地上传图片
        return __ROOT__.$url;
    }
...
{
    $admin=Db::name('admin')->where('member_id',$member_id)->find();
    if($admin){
        return true;
    }else{
        return false;
 ...
 * @param string $join 如 'member'不含前缀形式
 * @param string $joinon 'a.news_auto =b.member_id'字符串形式,$table为a表,$join为b表
...
{
    $links=Db::name("plug_link")->where(array('plug_link_typeid'=>$type,'plug_link_open'=>1))->order("plug_link_order ASC")->select();
    return $links;
}
/**
 * 根据广告位获取所有广告
 * @param int $ad_typeid 广告位id
 * @param int $limit
 * @param string $order
 * @return array|mixed;
 */
function get_ads($ad_typeid,$limit=5,$order = "ad_order ASC")
{
    if($order == ''){
        $order = "ad_order ASC";
    }
    ...
    return Db::name("ad")->where(array('ad_open'=>1,'ad_typeid'=>$plug_ad_adtypeid))->order($order)->limit('0,'.$limit)->select();
}
/**
 * 获取评论
 * @param string $tag
...
function get_comments($tag="field:*;limit:0,5;order:createtime desc;",$where=array())
{
    ...
    $comments=Db::name("comments")->alias("a")->join(config('database.prefix').'member b','a.uid =b.member_id')->field($field)->where($where)->order($order)->limit($limit)->select();
    return $comments;
}
/**
 * 查询文章列表，支持分页或不分页
 * @param string $tag  查询标签，以字符串方式传入,例："cid:1,2;field:title,content;limit:3,8;order:time desc,hits desc;where:n_id>5;"
 ...
 * @return array|mixed
 */
function get_article($tag,$ispage=false,$pagesize=10,$type=null,$v=null,$where=array(),$currentPage=null)
{
    $where=is_array($where)?$where:array();
    $tag=param2array($tag);
    $field = !empty($tag['field']) ? $tag['field'] : '*';
    ...
    switch($type){
        case 'keyword':
            $where['title|key'] = array('like','%' . $v . '%');//关键字
            break;
        case 'flag':
            $where['flag'] = array('like','%' . $v . '%');//标签
            break;
        case 'cid':
            $cid=intval($v);
    ...
    $where['back'] = array('eq',0);
    if (!empty($tag['cid'])) {
        $where['columnid'] = array('in',$tag['cid']);
    }
    if (!empty($tag['ids'])) {
    ...
    if($ispage){
        ...
        $count=Db::name("article")->field($field)->where($where_str)->where($where)->count();
        $article=Db::name("article")->alias("a")->join(config('database.prefix').'member b','a.auto =b.member_id')
        							->field($field)->where($where_str)->where($where)->order($order)->paginate($pagesize,false,$config);
        $show=$article->render();
        $content['page']=$show;
        ...
        //不使用分页
        $article=Db::name("article")->alias("a")->join(config('database.prefix').'member b','a.auto =b.member_id')
        							->field($field)->where($where_str)->where($where)->order($order)->limit($limit)->select();
        return $article;
    }
}
/**
 ...
 * @return array|mixed
 */
function get_product($tag,$ispage=false,$pagesize=10,$type=null,$v=null,$where=array(),$currentPage=null)
{
    $where=is_array($where)?$where:array();
    $tag=param2array($tag);
    $field = !empty($tag['field']) ? $tag['field'] : '*';
    ...
    if($ispage){
        //使用分页
        $pagesize=$pagesize?$pagesize:config('paginate.list_rows');
        $count=Db::name("product")->field($field)->where($where_str)->where($where)->count();
        $article=Db::name("product")->alias("a")->join(config('database.prefix').'member b','a.auto =b.member_id')->field($field)->where($where_str)->where($where)->order($order)->paginate($pagesize,false,$config);
        $show=$article->render();
        $content['page']=$show;
        $content['article']=$article;
        $content['count']=$count;
        ...
        //不使用分页
        $article=Db::name("product")->alias("a")->join(config('database.prefix').'member b','a.auto =b.member_id')->field($field)->where($where_str)->where($where)->order($order)->limit($limit)->select();
        return $article;
    }
}
