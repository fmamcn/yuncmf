...
class Admin extends Base
{
	...
	public function admin_del()
	{
		...
		//删除对应会员
		if($member_id){
			Db::name('member')->delete($member_id);
		}
	...
	public function admin_group_runadd()
	{
		if (!request()->isAjax()){
			...
			if($rst!==false){
				$this->success('管理组添加成功',url('admin/Admin/admin_group_list'));
			}else{
				$this->error('管理组添加失败',url('admin/Admin/admin_group_list'));
			}
		}
	}
	/**
	...
	public function admin_group_del()
	{
		...
		if($rst!==false){
			$this->success('管理组删除成功',url('admin/Admin/admin_group_list'));
		}else{
			$this->error('管理组删除失败',url('admin/Admin/admin_group_list'));
		}
	}
	/**
	 * 显示编辑页面
	...
	public function admin_group_runedit()
	{
		if (!request()->isAjax()){
			...
			$this->success('管理组修改成功',url('admin/Admin/admin_group_list'));
		}
	}
	/**
	 * 管理组状态
	...
	public function profile()
	{
		...
		if(session('admin_auth.aid')){
			...
			$news_count=Db::name('Article')->where(array('auto'=>session('admin_auth.member_id')))->count();
