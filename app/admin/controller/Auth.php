...
class Auth
{
	...
	public function getGroups($uid)
	{
		...
		if (isset($groups[$uid]))
			return $groups[$uid];
		$user_groups = db()->name($this->_config['auth_group_access'] . ' a')->where("a.uid='$uid' and g.status='1'")->join(config('database.prefix')."{$this->_config['auth_group']} g"," a.group_id = g.id")->field('uid,group_id,title,rules')->select();
	...
	protected function getUserInfo($uid)
	{
		...
		if(!isset($userinfo[$uid])){
			 $userinfo[$uid] = db()->where(array('admin_id'=>$uid))->name($this->_config['auth_user'])->find();
		}
		return $userinfo[$uid];
	}
}