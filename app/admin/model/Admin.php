...
class Admin extends Model
{
    ...
    public function auto_login($user, $rememberme = false)
    {
		...
		cookie('logged_user', jiami("{$auth['member_id']}.{$auth['admin_last_time']}"));
		$member=Db::name('member')->where('member_id',$auth['member_id'])->find();
    ...
    public static function add($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$group_id=1)
    {
        ...
        if($admin){
            ...
            //添加会员
            $member_id=Member::add($admin_username,$admin_pwd_salt,$admin_pwd,1,$admin_realname,$admin_email,$admin_tel,1,1);
            //修改admin对应member_id
