...
class YunCMF extends Taglib
{

    // 标签定义
    protected $tags = [
        // 标签定义： attr 属性列表 close 是否闭合（0 或者1 默认1） alias 标签别名 level 嵌套层次
		//'attr'=>返回的数组变量、文章ids、查询字段、limit、order、字符串条件、是否分页、单页数、搜索类型、搜索的值;具体参考get_article函数
        'article' => ['attr' => 'name,ids,cid,field,limit,order,where,ispage,pagesize,type,key', 'close' => 0],
		...
		//album
		'album' => ['attr' => 'name,pid,cid,field,order,limit', 'close' => 0],
    ];
    ...
    public function tagAlbum($tag)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .='$'.$name .'=get_menu_album('.'"'.$tag_str.'");';
        $parseStr .="?>";
        if (!empty($parseStr)) {
            return $parseStr;
        }
    ...
    public function tagArticle($tag)
    {
        ...
        if(substr($tag_str, 0, 1)==';') $tag_str=substr($tag_str,1);
        $parseStr = '<?php ';
        $parseStr .='$'.$name .'=get_article('.'"'.$tag_str.'",'.$ispage.','.$pagesize.',"'.$type.'","'.$key.'");';
        $parseStr .="?>";
        if (!empty($parseStr)) {
            return $parseStr;
        }
    ...
    public function tagMenu($tag)
    {
        ...
        $fixed_ul_class=isset($tag['fixed_ul_class']) ? $tag['fixed_ul_class'] : '';
        ...
        $parenttpl='<a href=\'\$href\' class=\''.$haschild_a_class.'\'>\$menu_name<span class=\''.$haschild_span_class.'\'>&nbsp;<i class=\'fa fa-angle-down\'></i></span></a>';
        $parseStr = '<?php ';
    ...
    {
        $name   = $tag['name'];
		$table    = isset($tag['table']) ? $tag['table'] : 'article';//'article'不含前缀形式
		$join    = isset($tag['join']) ? $tag['join'] : '';//'member'不含前缀形式
		//'a.auto =b.member_id'字符串形式,$table为a表,$join为b表
		...
		$where_str    = isset($tag['where']) ? $tag['where'] : '';//sql字符串形式"open=1 and title='aa'"形式
