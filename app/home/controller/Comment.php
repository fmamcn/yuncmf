...
class Comment extends Base
{
    ...
    public function runcomment()
    {
		...
		if (request()->isPost()){
            ...
            $data['uid']=$this->user['member_id'];
            $data['to_uid']=input('to_uid',0,'intval');
            $data['full_name']=$this->user['member_username'];
            $data['email']=$this->user['member_email'];
			...
			if ($result!==false){
				...
				$table_model->where(array($pk=>$data['t_id']))->update(array("comment_count"=>array("exp",Db::raw('comment_count+1'))));
				//更新path字段
				if($data['parentid']){
